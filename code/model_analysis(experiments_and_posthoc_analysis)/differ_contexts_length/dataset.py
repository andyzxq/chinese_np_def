...
class mydata(object):
    ...
    def _getdata(self, filename):  # 加载每行数据及其标签
        ...
        np_s = []
        np_e = []
        for lyq in range(len(read_data)):
            # print(len(read_data))
            # print(read_data[0])
            # 数据由4部分组成，文本、标签、NP起始位置、NP终止位置
            
            # 合并上下文
            line = ["", read_data[lyq][1], read_data[lyq][2]]
            for i in range(self.context_num):
                index = lyq - (self.context_num - i)
                if index >= 0:
                    line[0] += read_data[index][0].replace("* ", "").replace(" *", "") + " "
                    
            line[0] += read_data[lyq][0] + " "
            
            for i in range(self.context_num):
                index = lyq + (1 + i)
                if index < len(read_data):
                    line[0] += read_data[index][0].replace("* ", "").replace(" *", "") + " "
